...
import { supabase, type PersonRecord } from '@/lib/supabase';
import { useImageUpload } from '@/lib/useImageUpload';
import PersonFormModal from '@/components/PersonFormModal';
import {
  Plus, Search, Pencil, Trash2, Loader2, UserCircle2, Users, FileText,
} from 'lucide-react';
...
export default function PersonRecordsPage({ onNavigate }: { onNavigate: (tab: 'persons' | 'tasks') => void }) {
  ...
  const { publicUrl, remove } = useImageUpload();
  const [taskCounts, setTaskCounts] = useState<Record<string, { total: number; status: string }>>({});
  ...
  const load = async () => {
    ...
    const rows = (data ?? []) as PersonRecord[];
    setRecords(rows);

    // load task counts per person
    if (rows.length) {
      ...
      for (const t of (tasks ?? []) as { person_id: string; status: string }[]) {
  ...
  const handleDelete = async () => {
    ...
    await supabase.from('person_records').delete().eq('id', confirmDel.id);
    setConfirmDel(null);
    load();
  };
          ...
          {filtered.map((r) => {
            ...
            return (
              ...
              >
                ...
                >
                  <img
                    src={publicUrl(r.document_face_img_url)}
                    alt={r.full_name}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-emerald-950/40 p-4 backdrop-blur-sm" onClick={() => setViewing(null)}>
          <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl sm:flex-row" onClick={(e) => e.stopPropagation()}>
            <div className="sm:w-2/5 bg-emerald-50">
              <img src={publicUrl(viewing.document_face_img_url)} alt={viewing.full_name} className="h-full w-full object-cover" />
            </div>
