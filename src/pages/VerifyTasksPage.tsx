...
import { supabase, type PersonRecord, type VerifyTaskWithPerson } from '@/lib/supabase';
import { useImageUpload } from '@/lib/useImageUpload';
import VerifyTaskFormModal from '@/components/VerifyTaskFormModal';
import {
  Plus, Search, Pencil, Trash2, Loader2, ClipboardCheck, ExternalLink, ImageIcon,
} from 'lucide-react';
...
export default function VerifyTasksPage() {
  ...
  const { publicUrl } = useImageUpload();

  const load = async () => {
    setLoading(true);
    const [pRes, tRes] = await Promise.all([
      supabase.from('person_records').select('*').order('created_at', { ascending: false }),
      supabase
        .from('verify_tasks')
        .select('*, person_records(id, full_name, document_no)')
        .order('created_at', { ascending: false }),
    ]);
    setPersons((pRes.data ?? []) as PersonRecord[]);
    setTasks((tRes.data ?? []) as VerifyTaskWithPerson[]);
    setLoading(false);
  };
  ...
  const handleDelete = async () => {
    ...
    await supabase.from('verify_tasks').delete().eq('id', confirmDel.id);
    setConfirmDel(null);
    load();
  };
                ...
                {filtered.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="transition hover:bg-emerald-50/40">
                      ...
                      <td className="px-4 py-3">
                        {t.image_url ? (
                          <img src={publicUrl(t.image_url)} alt="face" className="h-12 w-12 rounded-lg object-cover ring-1 ring-emerald-100" />
                        ) : (
