...
import { supabase, type PersonRecord, type VerifyTask } from '@/lib/supabase';
...
export default function VerifyTaskFormModal({ open, onClose, editing, persons, saved }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        const { error: err } = await supabase.from('verify_tasks').update(payload).eq('id', editing.id);
        if (err) throw err;
      } else {
        const { error: err } = await supabase.from('verify_tasks').insert(payload);
        if (err) throw err;
      }
      saved();
      onClose();
